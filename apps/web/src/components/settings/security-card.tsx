...
function ChangePasswordForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (newPassword !== confirmPassword) {
      setError({ code: "PASSWORD_MISMATCH", message: "The new passwords do not match." });
      return;
    }
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      {success && (
        <p role="status" className="text-sm text-gold">
          Password changed. Other devices have been signed out; this one stays signed in.
        </p>
      )}
...
function ActiveSessions() {
  ...
  if (state.status === "error") {
    return <ErrorState error={state.error} onRetry={state.reload} title="Could not load sessions" />;
  }
  ...
  return (
    <div>
          ...
          >
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-xs text-muted">Signed in {formatDateTime(session.created_at)}</p>
              </div>
            </div>
            {session.is_current && (
      ...
      {otherCount > 0 && (
        ...
        >
          Log out other devices ({otherCount})
        </Button>
      )}
      {revoked && (
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Change your password and manage where you&apos;re signed in.</CardDescription>
      </CardHeader>
      <CardContent className="flex flex-col gap-6">
        <ChangePasswordForm />
        <div className="border-t border-white/10 pt-6">
          <p className="mb-3 text-sm text-ivory">Active sessions</p>
          <ActiveSessions />
        </div>
      </CardContent>
    </Card>
