...
export function EvidenceLayerContent({ personId, personName }: { personId: string; personName: string }) {
  ...
  async function createEntry() {
    if (checking || savingAnalysis || deletingEntries.size > 0) return;
    if (!METRICS.some((metric) => scores[metric])) { setEntryError(t("app.evidence.metricRequired")); return; }
    setSavingEntry(true); setEntryError(null);
    const body: LifeTrackingEntryCreateRequest = {
      entry_date: entryDate, note: note.trim() || null, custom_metrics: {},
  ...
  async function saveAnalysis() {
    if (!resolved || savingEntry || deletingEntries.size > 0) return;
  ...
  async function deleteEntry(entryId: string) {
    if (deletingEntries.has(entryId) || checking || savingEntry || savingAnalysis) return;
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1.05fr)_minmax(22rem,0.95fr)]">
      <section className="space-y-5" aria-labelledby="journal-title">
        ...
        <Card><CardContent className="space-y-5 p-5">
          ...
          <Button onClick={() => void createEntry()} loading={savingEntry} disabled={checking || savingAnalysis || deletingEntries.size > 0}><CalendarDays className="h-4 w-4" />{t("app.evidence.saveDay")}</Button>
        </CardContent></Card>
        <div className="space-y-3">{shownEntries.length === 0 ? <p className="rounded-xl border border-dashed border-white/15 p-6 text-sm text-muted">{t("app.evidence.empty")}</p> : shownEntries.map((item) => <Card key={item.id}><CardContent className="p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-medium text-ivory">{formatIsoDate(item.entry_date)}</p><div className="mt-2 flex flex-wrap gap-2">{METRICS.filter((metric) => item[metric] !== null).map((metric) => <Badge key={metric} variant="neutral">{t(metricKeys[metric])} {item[metric]}/10</Badge>)}</div>{item.note ? <p className="mt-3 text-sm text-text">{item.note}</p> : null}</div><Button variant="ghost" size="sm" loading={deletingEntries.has(item.id)} disabled={deletingEntries.has(item.id) || checking || savingEntry || savingAnalysis} aria-label={t("app.evidence.deleteEntry")} onClick={() => void deleteEntry(item.id)}><Trash2 className="h-4 w-4" /></Button></div></CardContent></Card>)}</div>
      </section>
      <section className="space-y-5" aria-labelledby="pattern-title">
        ...
        <Card><CardContent className="grid gap-4 p-5 sm:grid-cols-3 xl:grid-cols-1 2xl:grid-cols-3">
          <div><Label htmlFor="pattern-metric">{t("app.evidence.metric")}</Label><Select id="pattern-metric" value={query.metric_key} disabled={checking || savingAnalysis} onChange={(event) => setQuery((old) => ({ ...old, metric_key: event.target.value }))}>{METRICS.map((metric) => <option key={metric} value={metric}>{t(metricKeys[metric])}</option>)}</Select></div>
          ...
          <Button className="sm:col-span-3 xl:col-span-1 2xl:col-span-3" onClick={() => void checkPattern()} loading={checking} disabled={savingEntry || savingAnalysis || deletingEntries.size > 0}><Sparkles className="h-4 w-4" />{t("app.evidence.check")}</Button>
        </CardContent></Card>
        {analysisError ? <p role="alert" className="text-sm text-danger">{analysisError}</p> : null}
        {resolved ? <ResultCard result={resolved.result} onSave={() => void saveAnalysis()} saving={savingAnalysis} saveDisabled={savingEntry || deletingEntries.size > 0} /> : null}
