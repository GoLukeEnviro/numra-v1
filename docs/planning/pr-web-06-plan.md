...
## Verbindliche Pr�zisierungen aus der WEB-06a-�bergabe

Die folgenden Festlegungen ersetzen abweichende Empfehlungen/IST-Angaben weiter unten:

1. Eigene Idempotenz-Tabelle mit Schl�ssel `(Akteur, Workspace, Operation, Key)`;
   kanonischer Payload-Hash wird separat verglichen. Start **und** Submit ben�tigen
   den Header. Replay liefert den aktuellen autorisierten Zustand derselben Runde,
   auch nach Abschluss. Nach Dissolve werden s�mtliche POST-Replays abgewiesen.
2. Workspace-Row-Lock, frisch geladener Zustand und erneute Mitgliedschaftspr�fung
...
3. Tats�chlicher Typwechsel bei offener Runde: `CHECKIN_ROUND_OPEN`; gleicher Typ:
...
4. Template-Version ist ab erster Runden-Nutzung eingefroren. Config-Sperre deckt
   ...
   dieselbe fachliche Identit�t in der aktuellen Version.
5. Altrunden: `ANALYZED` beh�lt Analyse/Antworten unver�ndert, ohne erfundene
   Snapshots. `AWAITING` wird vorab auf vollst�ndige, passende Abgaben gepr�ft.
   Kompatible Backfills tragen ausdr�cklich `MIGRATION_CURRENT`, niemals
   `ROUND_START`. `snapshot_recorded=true` allein beweist keinen Original-Snapshot.
   Inkompatible Altrunden f�hren zum atomaren Abbruch; kein stilles Reparieren.
...
7. Fehlerantworten spiegeln keine eingereichten Rohwerte; Schemafehler werden f�r
   Check-in-Routen reduziert. Die Analyse-Privacy bleibt beim akzeptierten Trade-off.

Vollst�ndiger finaler API-Contract: `specs/v2/checkin-spec.md`, Abschnitt WEB-06a.
Migration und Pr�fprotokoll: `docs/planning/pr-web-06a-evidence.md`.
