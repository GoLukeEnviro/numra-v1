...
- **VERIFIED_MAIN_SHA:** `d7ebf36bbd6a7150739920f2f048d1e4015dfed9` (PR #190 — the last merge of the closure batch; the a11y fix #189 and this documentation merge precede it, full CI set incl. the required `sast` gate green)
- **VERIFIED_PRODUCTION_SHA:** `f522067e8521d78335b3a4c3ec75739809ce1a32` — read from `/var/lib/numra/deployed_sha` on the host 2026-09-22, which is the authoritative source. **Corrected:** this field previously named `b0ff3380`, which is an *ancestor* of the deployed commit; production is 12 commits ahead of that value and 103 commits behind `main`. No auto-deploy on this host — production advances only by an explicit deploy.
- **LAST_MERGED_PR:** [#190](https://github.com/GoLukeEnviro/numra-v1/pull/190)
- **LAST_GREEN_MAIN_RUN:** post-merge runs on `342fafcf` (#189, 16/16) and `d7ebf36b` (#190, 16/16)
- **CURRENT_MILESTONE:** PWA Product Closure
- **CURRENT_TASK:** PWA Product Closure. **2026-09-21 night, verified on the host:** the SAST gate (#140, PR #167) is merged AND bound as a required check; the **Personal Copilot** slice is merged (API PR #170 `9070ca3`, UI PR #178 `06cd97d`) and live-accepted on the audit stack with the **real** provider (thread 201, reply `COMPLETE` 947 chars, `model_provider=ollama_cloud`, cross-account 404, unauthenticated 401); the audit stack was rebuilt to `06cd97d` (parity 9/9, Alembic head `b9c0d1e2f3a4`) and the system journey (login → person → report job → DOM → PDF export → delete-all) passes; QUICK and FULL reports are proven with real generation (FULL 7 159/7 411 words), ULTIMATE failed its linter once (medical-claim rule) and retried visibly. **2026-09-21 closure batch, verified on the host:** the Personal Copilot verification/closure is **done** (#123 closed) and the two #174 halves are merged and verified — #176 (a normalisation refusal is now a 422 domain error, was HTTP 500) as PR #182 `d055d48`, the failed-turn visibility as PR #183 `126e2d8`, the corrective retry as PR #184 `4b0926c`. The audit stack was rebuilt to `4b0926cd` (Python parity 10/10) and the acceptance re-run on it: personal copilot reply COMPLETE 382 chars via the real provider, 422 on the name defect, cross-account 404, unauthenticated 401. **2026-09-22 PWA-10 closure batch, verified on the host:** the a11y finding is fixed and merged (#185, PR #189 `342fafc` — on `/login` the only h1 sat in a `hidden lg:block` panel and was display:none below `lg`; `/forgot-password` and `/reset-password` had none; each page now carries one `sr-only` h1 outside any hideable container, pinned by `public-page-headings.test.tsx`, RED 3/4 → GREEN 4/4). The offline question is decided and versioned (#186, PR #190 `d7ebf36b` — ADR 014 keeps navigation network-only so a shared device can never be served another person's cached page; `sw-navigation-strategy.test.ts` pins the cache allowlist and the network-only navigation, and the PWA-10 probe asserts the decision). The production origin allowlist was verified end to end (#187 closed): production origin → 401 (guard passes), foreign origin → 403 `ORIGIN_NOT_ALLOWED`; the check is now step 7 of the release runbook. The closure acceptance ran on the final SHA (`pwa10-closure-20260922T144716Z`, `VERDICT=PASS`, `findings: []`) through the browser path: register/person/calculation/thread 201, copilot turn `COMPLETE` 870 chars with `model_provider=ollama_cloud` read from the database. The state document's stale "Not yet a closure claim" section was corrected against the repo. **2026-09-22 PWA-05 external delivery, verified on the host:** the audit stack now sends through the real relay (`EMAIL_BACKEND=smtp` against `smtp.resend.com:587` with STARTTLS; `SmtpEmailSender` confirmed in the running app); the acceptance run `20260922T205442Z` returned `VERDICT=PASS` with an empty finding list, and the delivered message carries the recipient's own verdicts (`spf=pass`, `dkim=pass header.i=@mail.avenyth.de header.s=resend`, `dmarc=pass`, `arc=pass`). Two mandate collisions were found and documented: Resend refuses `example.com` recipients (RFC 2606), and port 465 is unreachable from this network (587/STARTTLS used, 2465 verified as fallback). #121 closed; evidence in `docs/audits/2026-09-22-pwa-05-external-delivery.md`. **Open: the production composition has no SMTP mapping at all** (see OPEN_RELEASE_BLOCKERS) **and the operator gates** (production deploy with its smoke, audit teardown).**
- **NEXT_ACTION:** Work order after the 2026-09-22 PWA-10 batch (steps 1, 2, 3 and 5 are DONE; #185, #186 and #187 are closed): **(1)** production capability via the approved release path — run `docs/ops/release-verification.md` including step 7 (origin allowlist against the real proxy; a 403 on the own origin is a release blocker), deploy, smoke, then close the loop; **(2)** production SMTP needs TWO things, in this order: first the **composition gap** — `/opt/numra/compose.production.yml` maps no `EMAIL_BACKEND`/`SMTP_*`, so a key alone changes nothing; then its own relay key (never the audit one) and an explicit deploy go. The audit instance is done (#121 closed): external delivery proven including the recipient's own SPF/DKIM/DMARC verdicts; **(3)** after a green production smoke, write the final Go/No-Go and close PWA-10; **(4)** audit teardown ONLY on an explicit operator go (synthetic accounts, audit stack, volumes, temporary mappings — evidence and restore information stay). Follow-ups #159–#165 stay deliberately non-blocking.
- **OPEN_RELEASE_BLOCKERS:** **not none — and one of them is new engineering work, not a deployment decision.** Production is untouched and unreleased. Two blockers hold it: (1) **the production composition carries no SMTP mapping at all** — `/opt/numra/compose.production.yml` has zero `EMAIL_BACKEND`/`SMTP_*` entries and is a standalone file with no `include:`/`extends:`, so a production SMTP key alone would not make production send; the audit stack gets its wiring from `docker-compose.yml` (PR #180), which production does not use. This is a composition gap to close before any deploy that is meant to deliver mail. (2) the **operator gates** (production deploy with its smoke, audit teardown). Every repo-side product finding is closed — #174, #123, #140, #185, #186, #187 — and PWA-05 is externally accepted on the audit instance. **Corrected 2026-09-22:** `VERIFIED_PRODUCTION_SHA` no longer names `b0ff3380` but the host's actual `deployed_sha`; see the field above. **Corrected 2026-09-21:** the earlier wording (audit without code parity) is resolved — the audit stack runs parity to its deployed SHA, the real-provider acceptance ran against it, and the oracle distinction (`ollama_cloud` vs historical `mock-v1` rows) is documented in the acceptance evidence.
