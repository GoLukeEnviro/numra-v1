...
- **CURRENT_TASK:** PWA-07 — privacy/evidence acceptance. The account-deletion half (cascade, retention matrix, cleanup proof) is documented and enforced; the export half is blocked because the product has no account data export (#125). PWA-05 and PWA-06 wait on external providers (see their roadmap rows).
- **NEXT_ACTION:** Continue with the gates that need no operator input: PWA-09 (monitoring, sanitized deployment topology, backup/restore state, runbooks) and the provider-independent parts of PWA-06/PWA-07. Three external blockers are with the operator: an SMTP provider plus a controlled `@example.com` mailbox for PWA-05 (#121), an LLM provider in the **audit** stack for PWA-06 (#124 — note the provider is currently set in production only), and the `ALLOW_SELF_SIGNUP` product decision for PWA-08 (#115). Audit records stay untouched: only PWA-10 authorizes teardown and deletion.
...
| PWA-07 | Privacy/evidence acceptance: export, account deletion, cascade and cleanup proof | **in progress** — the retention/cascade matrix is now documented and enforced (`docs/audits/2026-09-20-pwa-07-retention-matrix.md` plus `test_account_deletion_cascade_matrix.py`), covering every table with a FK to `users.id` plus a global dangling-FK sweep. The export half is blocked: the only export is a report PDF, and `ExportType.JSON` is unreachable (#125) |
| PWA-08 | Security/test hardening: triage current alert, CSP, SAST and targeted frontend coverage | **in progress** — HSTS was missing on both live surfaces and the security headers were untested; both fixed and pinned by tests (#126, merged as #127). A dependency alert is triaged as unreachable (#128). Open: the `ALLOW_SELF_SIGNUP` product decision (#115) |
