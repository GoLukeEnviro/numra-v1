...
- **VERIFIED_PRODUCTION_SHA:** `b0ff3380be1765a1b438283cfdad4656c9890d73` (no auto-deploy on this host — deliberately not advanced since the migration; the audit stack carries the acceptance revision)
...
- Email verification/reset has no external delivery proof: no SMTP provider, sender
  domain or controlled mailbox is configured in any environment (#121). The full
  code and SMTP transport path is proven against a loopback sink; the vendor
  delivery is not. `avenyth.de` currently has **no MX records**, so the domain is not
  mail-capable as configured.
